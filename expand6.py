...
def expand_uses_recursive(node, doc, id_map):
    ...
    if node.tagName == "use":
        ...
        if transform_attr:
            ...
            # FIX: Correctly prepend the new transform to the old one.
            # SVG applies transforms from right to left, so the <use> transform
            # should be applied *after* the referenced element's transform.
...
# FIX: The logic has been updated to correctly combine transforms at each level.
...
def remove_duplicate_segments(segments_list, tolerance=0.01):
    """
    Removes duplicate segments from the list of all segments, considering
    a given tolerance. A segment is considered a duplicate if its normalized
    endpoints are the same as another segment's endpoints, within the tolerance.
    """
    unique_segments = set()
    deduplicated_list = []
    
    # Rounding factor to handle the tolerance
    decimal_places = int(math.log10(1/tolerance))

    for polygon_segments in segments_list:
        for seg in polygon_segments:
            p1, p2 = seg
            
            # Normalize the segment by consistent point order
            if p1[0] > p2[0] or (p1[0] == p2[0] and p1[1] > p2[1]):
                p1, p2 = p2, p1
            
            # Round the coordinates to the specified tolerance
            p1_rounded = (round(p1[0], decimal_places), round(p1[1], decimal_places))
            p2_rounded = (round(p2[0], decimal_places), round(p2[1], decimal_places))
            
            normalized_seg = (p1_rounded, p2_rounded)
            
            if normalized_seg not in unique_segments:
                unique_segments.add(normalized_seg)
                deduplicated_list.append(seg)
...
deduplicated_segments = remove_duplicate_segments(result, tolerance=5)
...
# M 418.84118742426085 347.6657939504258
# L 418.8430101065271 340.739617768617

# m 418.85794,340.72934 -0.002,6.92618