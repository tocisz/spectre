...
def create_svg_with_defs(paths, clusters, representatives, output_file):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        # Write defs with representative paths
        f.write("  <defs>\n")
        for i, rep_idx in enumerate(representatives):
            path_el = paths[rep_idx]
            # Clone the representative path but assign new id
            path_copy = path_el.cloneNode(True)
            path_copy.setAttribute("id", f"rep{i}")
            f.write(f"    {path_copy.toxml()}\n")
        f.write("  </defs>\n\n")

        # Write <use> for each path in big clusters
        for cluster_idx, cluster in enumerate(clusters):
            for p_idx in cluster:
                # Skip representative itself because it's already in defs
                transform = paths[p_idx].getAttribute("transform")
                style = paths[p_idx].getAttribute("style")
